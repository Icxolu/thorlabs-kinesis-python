from _thorlabs.motions_control.kcube.stepper_motor import ffi, lib
...
class KCubeStepperMotor(KinesisDevice):
    ...
    def identify(self):
        lib.SCC_Identify(self._serial_buffer)
    ...
    def get_software_version(self) -> str:
        v = lib.SCC_GetSoftwareVersion(self._serial_buffer)
    ...
    def get_is_calibration_active(self) -> bool:
        return lib.SCC_IsCalibrationActive(self._serial_buffer)

    def get_calibration_file(self) -> str:
        size = 100
        filename = ffi.new("char[]", size)
        while not lib.SCC_GetCalibrationFile(
    ...
    def set_calibration_file(self, filename: str, enabled: bool = True):
        ...
        lib.SCC_SetCalibrationFile(self._serial_buffer, filename, enabled)

    def get_hub_bay(self) -> int:
        return lib.SCC_GetHubBay(self._serial_buffer)

    def load_settings(self) -> bool:
        return lib.SCC_LoadSettings(self._serial_buffer)

    def load_named_settings(self, settings_name: str) -> bool:
        settings_name = ffi.new("char[]", settings_name.encode("utf-8"))
        return lib.SCC_LoadNamedSettings(self._serial_buffer, settings_name)
    ...
    def get_can_home(self) -> bool:
        return lib.SCC_CanHome(self._serial_buffer)
    ...
    def get_software_limit_mode(self) -> LimitsSoftwareApproachPolicy:
        return LimitsSoftwareApproachPolicy(
            lib.SCC_GetSoftLimitMode(self._serial_buffer)
        )
    ...
    def get_stage_axis_limits(self) -> TravelLimits:
        return TravelLimits(
            lib.SCC_GetStageAxisMinPos(self._serial_buffer),
            lib.SCC_GetStageAxisMaxPos(self._serial_buffer)
        )
    ...
    def clear_msg_queue(self):
        lib.SCC_ClearMessageQueue(self._serial_buffer)

    def get_msg_queue_size(self) -> int:
        return lib.SCC_MessageQueueSize(self._serial_buffer)

    def get_next_msg(self):
        msg_type = ffi.new("WORD *")
        msg_id = ffi.new("WORD *")
        msg_data = ffi.new("DWORD *")
        success = lib.SCC_GetNextMessage(
    ...
    def wait_for_msg(self):
        ...
        msg_id = ffi.new("WORD *")
        msg_data = ffi.new("DWORD *")
        success = lib.SCC_WaitForMessage(
            self._serial_buffer, msg_type, msg_id, msg_data
        )
        return success, msg_type[0], msg_id[0], msg_data[0]
    ...
    uses_pid_loop_encoding = property(get_uses_pid_loop_encoding)
    pid_loop_encoder_params = property(
        get_pid_loop_encoder_params, set_pid_loop_encoder_params
    )
    pid_loop_encoder_coefficient = property(
        get_pid_loop_encoder_coefficient, set_pid_loop_encoder_coefficient
    )
